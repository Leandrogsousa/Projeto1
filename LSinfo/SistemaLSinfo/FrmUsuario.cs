...
namespace SistemaLSinfo
{
    public partial class FrmUsuario : Form
    {
        ...
        {
                SqlConnection cn = new SqlConnection("Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=LSinfo;Data Source=LEANDRO_SOUSA");
            SqlCommand cmd = new SqlCommand("update Usuario set id_usuario =@id_usuario,nome =@nome,logins =@logins,senha =@senha,telefone =@telefone,email  =@email,perfil =@perfil where id_usuario = @id_usuario", cn);
                ...
                if (txtid.Text != "" & txtnome.Text != "" & txtlogin.Text != "" & txtsenha.Text != "" & txttelefone.Text != "" & txtemail.Text != "" & comboperfil.Text != "")
                {
                    try
                    {
                        cn.Open();
                        cmd.ExecuteNonQuery();
                        MessageBox.Show("Atualizado Com Sucesso!", "SISTEMA LSINFO - ALTERAR", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        txtid.Text = "";
                        txtnome.Text = "";
                        txtlogin.Text = "";
                        txtsenha.Text = "";
        ...
        {
            SqlConnection cn = new SqlConnection("Integrated Security=SSPI;Persist Security Info=False;Initial Catalog=LSinfo;Data Source=LEANDRO_SOUSA");
            SqlCommand cmd = new SqlCommand("delete from Usuario where id_usuario = @id_usuario", cn);
            cmd.Parameters.Add("@id_usuario", SqlDbType.VarChar).Value= txtid.Text;
            ...
            try
            {
                cn.Open();
                cmd.ExecuteNonQuery();
                MessageBox.Show("Dados Excluídos com Sucesso!", "SISTEMA LSINFO - EXCLUIR", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtid.Text = "";
                txtnome.Text = "";
                txtlogin.Text = "";
                txtsenha.Text = "";
        ...
        private void btbsairusuario_Click(object sender, EventArgs e)
        {
            this.Close();
        }
    }
